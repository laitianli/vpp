...
#define PCAP_DIR "/opt/pdump/"
#define PCAP_FILE "udp_fwd.pcap"
...
const char* open_close_udp_fwd_pcap(const char *pathname, u8 is_open, int cpuid)
{
	if (is_open == 1) {
...
static void* write_packet_thread(void* arg)
{
	...
	pthread_setname_np(pthread_self(), "udpfwd_pcap");
...
/* 将下行抓包接口，将报文存放fifo中 */
void udp_fwd_dl_pcap(u8* data, u32 len, u8 is_ip4)
{
	one_packet_data_t *pkt = NULL;
	struct timeval t;
...
/* 将上行抓包接口，将报文存放fifo中 */
void udp_fwd_ul_pcap(u8* data, u32 len, u8 is_ip4)
{
	one_packet_data_t *pkt = NULL;
	struct timeval t;
	ethernet_header_t* eth = NULL;
