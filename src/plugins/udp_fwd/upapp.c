...
#define LIBFILE "/home/haizhi/downland/vpp/src/plugins/udp_fwd/upapp/libupapp.so"
...
/* libupapp.so库从upapp进程改造而来，进程需要传输参数为：upapp -pdcpCoreIpAddress 192.0.2.2 -l2IpAddress1 192.0.2.10 -l2IpAddress2 192.0.2.11 -cellNumber 1 
 * 并且参数之间使用'\0'分隔。此函数的strsplit_with_delim()就是完成此功能
 */
...
/* 
 * 之所以创建一个线程是因为upapp进程的主线程是阻塞的，在改造成libupapp.so库后，为不影响vpp主线程运行，将libupapp.so中的libupapp_main()放到独立线程运行。
 */
